...
def getauthcodefns(url,env,username,password):
    ...
    __code=getCodeAuthfn(__auth02url,env)
    return  __code

def getCodeAuthfn(url1,env):

    if env=="devenv":
        __redirect_uri= devData()['admin']['redirect_uri']   
    elif env=="testenv":
        __redirect_uri= testData()['admin']['redirect_uri']   
    else: 
        __redirect_uri= devData()['admin']['redirect_uri']   
    ...
    __auth_url="https://auth."+env+".printix.net/oauth/token"
    ...
    #mylogger.info(__authorisation_code.status_code)
    assert __authorisation_code.status_code ==200
    #mylogger.info(__authorisation_code.json()['access_token'])
